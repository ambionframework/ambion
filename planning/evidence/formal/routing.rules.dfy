// Generated by lsc from routing.rules.ts
...
datatype Hold = running(since: int) | ended(since: int, until: int)
...
{
  match hold {
    case running(i_hold_since) =>
      (i_hold_since < seq_)
    case ended(i_hold_since, i_hold_until) =>
      ((i_hold_since < seq_) && (seq_ <= i_hold_until))
  }
}
...
{
  Person(entry.name, (match entry.identity { case Some(i_entry_identity_val) => i_entry_identity_val case None => (match known { case Some(i_known_val) => i_known_val.identity case None => "" }) }), Presence.present, (match known { case Some(i_known_val) => i_known_val.since case None => Option.None }), Some(entry.at), (match entry.preferences { case Some(i_entry_preferences_val) => Option.Some(i_entry_preferences_val) case None => (match known { case Some(i_known_val) => i_known_val.preferences case None => Option.None }) }))
}
