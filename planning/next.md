...
| M4. The workspace logs                              | #PRNUM           | The audit log reports a directory failure to `onError`. The logs share one record path, one path check, and one file match. SQL output uses one table      |
