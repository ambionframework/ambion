...
| Area                              | Acceptance                                                                                                                                                                                                                                      |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1 Definitions and executors      | A definition is a value with one executor. Executor families run in one room: the Pi loop, the Claude Agent SDK harness, and the Codex SDK. A fixed definition set per run; membership changes by name; a fixed seat that agents cannot remove. |
...
- **The kernel imports no model library.** The Claude Agent SDK and the Codex SDK are
  executor packages.
...
| C    | Phase 7: 8, then 9 last                                    | P1       |

**The critical path is phase 8.** Start step 1 first, because every
...
**Evidence:** the API reference builds in CI, and a stale reference fails
the build.
...
**C6. Small sharp edges.** Host `seat()` rejects a repeat while the agent
tool returns `unchanged`, and the `say` key and a human delivery key share
one key space. Make the host operation idempotent and prefix the key kinds.
...
**Ten published packages and one example.** Each package has
