...
> - **One rename is an accepted exception.** Pull request 224 renamed
>   `exchangeUri` to `messageUri`.
...
**The critical path is 6.3 and 7.7, then 8.** Start lane C first; when
hands run short, take lane A, because the adapters carry the release
story.
...
**C6. Small sharp edges.** `visit.send()` returns a handle whose `owner`
can be another person; host `seat()` rejects a repeat while the agent tool
returns `unchanged`; the `say` key and a human delivery key share one key
space. Add `opened` to the handle, make the host operation idempotent, and
prefix the key kinds.
...
them. Run the executor conformance suite on each with a fake: a scripted
