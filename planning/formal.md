...
rules files and two proofs files carry 123 obligations. The other groups
...
- **A4. A malformed seq.** `positionOf` skips a stored entry whose seq is
  ...
  for a known kind with a bad body, once the golden journals in
  `test/golden` show the change is safe.
...
on other kinds is a schema change that a later journal format takes.
...
| ----- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 1     | `journal.ts`                               | `fenceStep`, `keyed`, `writable`, `advanceSeq`, `scanned`, the fold lemmas; `admit`, `nextPosition`, `readPosition`, and `visibleEntries` were proven once and withdrawn                                                                                                                          | Landed    |
| 1     | the two cursor consumers                   | `scanned` at the Cloudflare and Pi cursors (A2)                                                                                                                                                                                                                                                   | Landed    |
| 2a    | `transition.ts`                            | `mayEnd`, `leaseExpiry`, `acknowledged`, `onRecord`, `speechFreshness`, `admitsClose`, `coversExchange`, `admitsLease`, `stampedSummary`; `permits`, `commitAuthority`, `addressesOwner`, and the presence, membership, address, `distinct`, and retry-match rules were proven once and withdrawn | Landed    |
| 2b    | `lease.ts`, `fold.ts`                      | `applyChange`, `cancelHold`, `answers`, `wakeAnswered`, `countsAgainst`, `nextActivationId`, `survivesCancellation`; `schedule`, `latest`, and `removedAfter` were proven once and withdrawn                                                                                                      | Landed    |
| 2c    | `reconcile.ts`                             | `endingOf`, `exchangeLive`; `staleLease`, `mayClose`, `waitsUntil`, `readyToSend`, `looksAgainAt`, `earliestAfter`, `forgets`, `closeMoved`, and `namesWriter` were proven once and withdrawn                                                                                                     | Landed    |
| 2d    | `routing.ts`, `delivery.ts`, `presence.ts` | Every rule, the steer included, was proven once and withdrawn                                                                                                                                                                                                                                     | Withdrawn |
...
| ------------------------------------------- | -------------------------------------------- | ----------------- | ------------------------------------------------- |
| A. Journal                                  | `packages/journal/src/rules.verified.ts`     | 12, and 23 proofs | Landed                                            |
| B, C, D1 to D7, D9, F, G1 to G3, G5         | `packages/ambion/src/room/rules.verified.ts` | 74, and 14 proofs | Landed                                            |
