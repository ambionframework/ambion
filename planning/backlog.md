...
**Fix.** Find what the wait is waiting for. `runDurableObjectAlarm` answers
whether an alarm was scheduled, and the test drops the answer: an alarm the
seat has not armed yet runs nothing, and the room's resend is what recovers
it. Read the answer, and say which of the two started the activation. Then
the failure names the step that did not happen.
