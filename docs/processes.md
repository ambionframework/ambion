...
carries that time. `bash` and `wait` wait for the shorter of the time they
ask for and the time left before the margin. A process that still runs then
gives `running`, and the result line adds `The wait stopped early, because
your activation ends in <n> seconds.` The agent can still answer before
the room ends the activation.
