...
| Git tokens on a workstation    | An agent reads its own `~/.git-credentials`, and it can copy a token into the record. Each token grants one scope on one repository, and it expires after `tokenTtl`. A new `secret` revokes every token.                                                                                              | [Git](git.md#trust)                                                                               |
