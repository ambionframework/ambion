...
seq from the database as the row lands. Pi's JSONL repository holds it in
memory, so two runs over one file write the same seq twice and the file no
longer reads. The fence then has nothing to read, and the record is lost.
