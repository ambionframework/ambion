...
message, and which is drafting one now are held by the aides themselves
...
once, and that message is the answer. **An aide does not engage.** The
...
to each other is nothing an aide consolidates. And it counts messages, not
...
The aide reads the record to `through` and drafts. At the moment it
...
summary lands, and the room is never held busy while an aide writes.
...
A room holds several people, and each may bring an aide. Only one writes
per exchange:
...
> — the owner stays, and so does which aide writes at the close.**
...
**An aide outlives its person's visit by one exchange.** Priya may ask and
walk out before the room settles. The exchange is still hers, it still
closes, and her aide still writes its summary — addressed to her, waiting
...
question opens his own exchange, and his own aide writes it.
...
nobody said it: a person did not hear it in a room; an aide wrote it.
...
its decisions in a transcript, and was fragile before an aide existed.
...
sees what she sees: the room thinking, then the answer it came to. This
holds whether or not Sam brought an aide of his own — folding follows the
summary that exists, whoever reads. The working is hidden from every
reader, because it was working, and there is no per-person view to keep
straight.
...
prints the span it stands for — `∎ priya-aide → priya (2–12)` — and leaves
...
author covers everything an aide writes, so one line enforces the whole of
§12's rule: nothing an aide writes can start work.

Every seat still **reads** everything an aide writes. Waking and reading
...
constant. One rule decides whether an aide is still an aide:
...
an aide carries no tools of its own, and `defineHuman` refuses one that
...
- **Causing work.** A room that woke because somebody's aide wanted
...
**A question from somebody with no aide is never summarised.** The exchange
it opens runs and closes with nothing written at the end of it, every
reader sees it whole, and no range leaves any seat's context. A room where
nobody brings an aide is a room an aide changes in no respect.

That is the guarantee this design offers: **it adds one message, and it
takes nothing away that anybody can still reach.**
...
its own span. `defineHuman` takes one optional field, the record holds one
more kind, a person's seat names the aide they brought and the aide's own
...
§16's symmetry rests on: an aide reads exactly what the seats read. A
...
`defineHuman` refuses an aide that carries tools of its own, so §12's
...
- **The aide holds no authority over the room.** §12 draws the line and
  names what is forbidden.
- **A person may have no aide.** Then nothing changes. §13.
...
lands while the aide drafts. That is seconds of room at most. It is the
...
**The room still chatters.** An aide makes ten messages readable. It does
...
the chatter is one run away, and it would change what an aide has to do.
...
- A person with no aide is never summarised, in a room whose prompt does
  not mention a fold. §13.
...
- An aide's turns land in a downstream session of its own, and the room
...
- `defineHuman` refuses an aide that holds tools or takes its person's
...
no aides ended at 8,266 characters for 29 messages against this run's 36,
so the two came out level. The room with aides in it said more, and
...
person it was written for; and §14's paragraph, which now asks an aide for
