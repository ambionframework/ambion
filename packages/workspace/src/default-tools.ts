 ...
 * of the guidance.
 *
 * Every workspace gets these three tools and the five process tools
 * (`./process-tools.ts`: bash, ps, status, wait and cancel) before any tool its
...
const COUNTS = ['eight', 'nine', 'ten', 'eleven'];

/** The tools every workspace has, in the order the tool line names them. */
const BASE_TOOLS = ['read', 'write', 'edit', ...PROCESS_TOOL_NAMES];
