...
describe.each(storages)('protocol value ownership on $name', (storage) => {
	...
	it('keeps error diagnostics deliverable when their causes cannot be cloned', async () => {
		...
		emit?.({ type: 'trace_error', agent: 'alpha', activation: 'audit', error });
		...
		expect(seen.find((event) => event.type === 'trace_error')).toMatchObject({
			agent: 'alpha',
			activation: 'audit',
		});
		expect(seen.find((event) => event.type === 'trace_error')?.error).toBe(error);
