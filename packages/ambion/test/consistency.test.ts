...
class Cluster {
	...
	/** Room calls the nemesis dropped, counted when taken: every one fails an activation, and that is one error. A dropped wake is sent again, a dropped steer is recovered from the journal, and a dropped cut leaves the record's word to stand, so all three fail nothing. */
	private dropped = 0;
	...
	failWire(): string {
		...
		const taken = () => {
			if (kind === 'drop' && on !== 'wake' && on !== 'steer' && on !== 'cut') this.dropped += 1;
			return true;
		};
