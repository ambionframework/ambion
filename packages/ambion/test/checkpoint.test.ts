 ...
 * which every wake was answered. The journal drops the rows it replaced, the
...
const rows = (journal: RoomJournal) => journal.entries.filter((entry) => entry.kind !== 'message');
...
describe('a checkpoint', () => {
	it('stands for the rows it replaces, and the journal drops them', async () => {
		...
		try {
			...
			expect(rows(journal).length).toBeGreaterThan(3);

			const row = checkpointOf(fold(journal), clock.now());
			expect(row).toBeDefined();
			if (row === undefined) return;
			await journal.write('checkpoint', row);
...
describe('a checkpoint the room folds', () => {
	it('holds a lease it carries, so a later end starts no second activation', async () => {
		...
		try {
			...
			// the checkpoint carries the live lease, and the journal drops the row it replaced
			const row = checkpointOf(fold(journal), 0);
			if (row === undefined) throw new Error('the room wrote no checkpoint');
...
describe('a checkpoint past the fence', () => {
	it('is void, and the journal folds the one that stood', async () => {
		...
		try {
			...
			expect(rows(journal).filter((entry) => entry.kind === 'checkpoint')).toHaveLength(1);
		} finally {
			await opened.dispose();
		}
	});
