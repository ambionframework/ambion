 ...
 * asserts each copy equals the public type in `types.ts`. `Message` is the
 * public union itself; the stub below names the four fields the rules read.
 */
...
export function markedCancelled(hold: Hold): boolean {
...
export function fromCatalog(catalog: readonly Seating[], seat: Seating): boolean {
