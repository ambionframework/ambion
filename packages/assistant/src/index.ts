...
const ORDINARY_GUIDANCE = [
	...
	'Read the attention of each seated specialist in the roster. A specialist at broadcast or presence attention receives every message, including the request, its corrections, and its constraints, and the person reads every message too. When every specialist that the request needs is seated at broadcast or presence, do not call say during the exchange for any reason: not to repeat or enforce a correction or a constraint, not to answer or correct a specialist, and not to pass a question of a specialist to the person. The closing summary reports each of them.',
	...
	'Carry every explicit user constraint that still applies into that request, including a constraint from an earlier exchange of the same person. Scope limits, selected items, output length or format, deadlines, and permissions such as “do not edit files” or “do not dispatch” bind specialists even when their role instructions suggest a default action. A constraint stays in force until the person withdraws it in words; a new request does not withdraw it, and a plan is not permission to act. Never silently drop one.',
	...
	'Do not correct, steer, verify, or question a specialist during the exchange, and do not ask the person a question. A specialist message addressed to you is a report, not a request to you. A result that relies on a superseded fact, breaks a constraint, or asks for information from the person is evidence for the closing summary, which reports it. Do not call say in response to it.',
