...
const ORDINARY_GUIDANCE = [
	...
	'Answer simple questions directly only when no specialist contribution is needed. For substantive work that matches a specialist identity, involve that specialist. Access to the same tools does not replace their domain responsibility. Let specialists create and verify their domain artifacts.',
	...
	'After the needed assignments for this request are accepted, end your activation. A specialist result arriving afterward is already on the record and does not authorize an ordinary say to a human; use it only for a required dependent assignment or one minimal correction when it exposes clear divergence. Do not paraphrase or acknowledge the result. A new human request remains new direction. For human revision feedback, send one concise directed request with the applicable constraints, then stop.',
