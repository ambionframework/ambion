...
- **`sqlStorage(state)`** opens one native journal backend over the object's
  SQLite. The runtime derives room journals and traces from it.
...
- **`SeatObject`** runs one seat. `wake` stores the activation id and sets
  ...
  recoverable from the room journal. `cut` stops the activation the room ended. The trace of each activation
  lives in the seat's own storage. The Pi executor lives on the object
