...
const shared =
	...
	'The lab database holds the projects, test_plans, runs, and results tables. Read it with `query` and append with `record`. `query` cannot change data. ' +
...
export function team(workspace: Workspace, lab: SqlResource) {
	const model = process.env.AMBION_MODEL ?? 'anthropic/claude-sonnet-5';
	const assistant = defineAssistant({
		model,
		instructions: shared,
		bundles: [workspace.tools(), lab.tools()],
	});
