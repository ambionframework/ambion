...
export function team(workspace: Workspace) {
	...
	const shared =
		...
		'Use task for independent background work while you continue to engage with the room. You can select yourself or specialists for its working room. Carry the human constraints into the assignment. Check the Tasks you own in this exchange before starting duplicate work. Background Tasks must settle before this exchange ends. ' +
	...
	const specialistDefinitions = specialists.map(({ instructions, ...definition }) =>
		defineAgent({
			...definition,
			instructions: `${shared}${instructions} For a Task pinned to your working room, report progress and the final result with task_update. For ordinary room work, report your result to assistant, or to the specialist who asked you. Reply once when your assignment is done. Stay silent on acknowledgments and when there is no new work.`,
			model,
			bundles: [workspace.tools()],
		}),
	);
