...
function wrapStatement(
	...
	return new Proxy(statement, {
		get(bound, method) {
			...
			if (method === 'run' && state.catalogFailure && query.includes('UPDATE demo_rooms'))
				return () => {
...
describe('persistent host stop recovery', () => {
	...
	it('retains a failed stop handle, retries concurrent requests, and gates admission', async () => {
		const directory = await mkdtemp(joinPath(tmpdir(), 'ambion-stop-recovery-'));
	...
	it('reopens a failed stop as running intent and retries it on the next host', async () => {
		const directory = await mkdtemp(joinPath(tmpdir(), 'ambion-stop-restart-'));
		directories.push(directory);
		const wrapped = wrappedDatabase(`${directory}/rooms.db`);
	...
	it('retries the catalog stop intent after cleanup succeeds but its save fails', async () => {
		const directory = await mkdtemp(joinPath(tmpdir(), 'ambion-stop-catalog-'));
		directories.push(directory);
		const wrapped = wrappedDatabase(`${directory}/rooms.db`);
		...
		try {
			...
			const rows = wrapped.database
				.prepare('SELECT enabled FROM demo_rooms WHERE name = ?')
	...
	it('reports failed HTTP stop cleanup and rejects admission until retry succeeds', async () => {
		...
		let demo: Awaited<ReturnType<typeof openDemo>> | undefined;
		try {
			demo = await openDemo(joinPath(directory, 'demo'), 'start', stream);
			...
			const first = await request(base, '/rooms/design/stop', { method: 'POST' });
			...
			expect((await request(base, '/rooms')).body).toEqual(
				expect.arrayContaining([expect.objectContaining({ name: 'design', status: 'stopping' })]),
			);
			failure.restore();
			const second = await request(base, '/rooms/design/stop', { method: 'POST' });
			expect(second.response.status).toBe(200);
			expect((second.body as { status: string }).status).toBe('stopped');
			const messages = await request(base, '/rooms/design/messages');
			expect(
			...
			try {
				expect(
					(await request(base, '/rooms/design/resume', { method: 'POST' })).response.status,
				).toBe(200);
				expect((await request(base, path, { method: 'PUT' })).response.status).toBe(200);
				await expect(demo.close()).rejects.toThrow(/write failure/);
			} finally {
